...
@Injectable()
export class PracticeProgramAccessGuard implements CanActivate {
  private readonly logger = new Logger(PracticeProgramAccessGuard.name);
  constructor(
  ...
  /**
   * Using the @Roles decorator will check if the current user has access assigned and has access to the practice 4 good application
   * @param context
   */
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 3. For Admin and Employee we need to validate if he has the practice program application
    try {
