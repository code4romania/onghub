...
class Client {
    ...
    constructor(apiKey, secret, settings) {
        ...
        if (typeof settings !== 'object') {
            throw new AuthError_1.default('Client created with invalid settings.');
        }
        ...
        if (apiKey === undefined || apiKey === null || apiKey === '' || typeof apiKey !== 'string') {
            throw new AuthError_1.default('Client created without an API key.');
        }
        ...
        if (!sign_1.algorithms.includes(validatedSettings.algorithm)) {
            throw new Error(`Invalid HMAC algorithm: "${settings.algorithm}". The only supported algorithms are: "${sign_1.algorithms.join('", "')}"`);
        }
        ...
        if (settings.ssl) {
            this.agent = new https_1.default.Agent({ maxSockets: settings.maxSockets });
        }
        else {
            this.agent = new http_1.default.Agent({ maxSockets: settings.maxSockets });
        }
    ...
    request(call, callback) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let httpLibrary;
            if (ssl === true) {
                httpLibrary = https_1.default;
            }
            else {
                httpLibrary = http_1.default;
            }
                ...
                response.addListener('end', () => {
                    ...
                    if (response.statusCode !== 200) {
                        ...
                        if (response.statusCode === 401) {
                            error = new AuthError_1.default(`Error ${response.statusCode}`);
                        }
                    ...
                    if (typeof callback === 'function') {
                        callback(undefined, object !== null && object !== void 0 ? object : responseData);
                        callback = undefined;
                        return;
                    }
